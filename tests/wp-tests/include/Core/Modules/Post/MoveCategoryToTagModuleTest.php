 ...
 */
class MoveCategoryToTagModuleTest extends WPCoreUnitTestCase {

	/**
	 * @var \BulkWP\BulkMove\Core\Modules\Post\MoveCategoryToTagModule
	 */
	...
	public function setUp() {
		...
		$this->category_to_tag_metabox = new MoveCategoryToTagModule();
	}
	...
	public function test_move_posts_from_category_to_tag_with_overwrite() {
		...
		// Assert that common tag has two posts.
		$posts_in_common_tag = $this->get_posts_by_tag( $common_tag );
		$this->assertEquals( 2, count( $posts_in_common_tag ) );
	}
}
