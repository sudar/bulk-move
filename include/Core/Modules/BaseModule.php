...
abstract class BaseModule {
	...
	protected function render_tags_dropdown( $name, $tags = array(), $show_option_none = false ) {
		...
		if ( count( $tags ) >= $bm_select2_ajax_limit_tags ) {
			?>
			<select class="select2Ajax" name="<?php echo sanitize_html_class( $name ); ?>" data-term="post_tag"
			        data-placeholder="<?php _e( 'Select Tag', 'bulk-move' ); ?>" style="width:300px">
			</select>
		<?php } else {
			?>
	...
	protected function render_post_type_dropdown() {
		?>
		<select name="smbm_<?php echo esc_attr( $this->action ); ?>_post_type"
		        id="smbm_<?php echo esc_attr( $this->action ); ?>_post_type">
